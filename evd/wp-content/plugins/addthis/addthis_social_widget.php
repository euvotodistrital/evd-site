...
$addthis_new_styles = array(
    ...
    'fb_tw_sc' => array( 'src' => '<div class="addthis_toolbox addthis_default_style last " %s  ><a class="addthis_button_facebook_like" fb:like:layout="button_count"></a><a class="addthis_button_tweet"></a><a class="addthis_button_google_plusone" g:plusone:size="medium"></a><a class="addthis_counter addthis_pill_style"></a></div>' , 'img' => 'fb-tw-sc.jpg' , 'name' => 'Like, Tweet, Counter', 'above' => '', 'below' => 'hidden'
...
function addthis_display_social_widget($content, $filtered = true, $below_excerpt = false)
{
    ...
    //don't display the addthis to the slideshow
...
/**
 * Função para imprimir o widget sem precisar anexar ao conteúdo do post
 *
 * return string - código html para imprimir widget
 */
function addthis_call_social_widget()
{
    global $addthis_styles, $addthis_new_styles;
    $styles = array_merge($addthis_styles, $addthis_new_styles);
    ...
    $url = get_permalink();
    $title = get_the_title();
    $url_final =  "addthis:url='$url' ";
    $url_final .= "addthis:title='$title'";
    
    $widget = '';

    if (isset ($styles['fb_tw_sc'])) {
        $widget = apply_filters('addthis_below_content', $styles['fb_tw_sc']['src']);
    }

    $result = sprintf($widget, $url_final);
