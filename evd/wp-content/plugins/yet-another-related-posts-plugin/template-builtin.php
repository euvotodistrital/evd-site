...
if ($related_query->have_posts()) {
	while ($related_query->have_posts()) {
		...
		if (current_user_can('manage_options') && $domain != 'rss')
			$output .= ' <abbr title="'.sprintf(__('%f is the YARPP match score between the current entry and this related entry. You are seeing this value because you are logged in to WordPress as an administrator. It is not shown to regular visitors.','yarpp'),round(get_the_score(),3)).'"></abbr>';
		$output .= '</a>';
		if ($show_excerpt) {
			$output .= $before_post .
